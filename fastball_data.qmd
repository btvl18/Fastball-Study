...
$$\text{logit}(\omega) = \beta_0 + \beta_1*\text{velo}+\beta_2*\text{spin_rate}+\beta_3*\text{extension}+\beta_4*\text(\text{arm_angle}*\text{ivb})+\beta_5*\text{arm_angle}+\beta_6*\text{ivb}$$
...
post_omegas <- fastball_model_fit |>
  spread_draws(b0, b1, b2, b3, b4, b5, b6, kappa2)
...
# Generate predicted omega values and simulated whiff rates
posterior_pred <- post_omegas |>
  crossing(fastball_data) |>
  mutate(
    omega = inv_logit(
      b0 + b1 * velo +
      b2 * spin_rate +
    ...
    sim_whiff_rate = rbeta(n(), shape1 = alpha, shape2 = beta)
  )
```
...
```{r}
gf_dens(~ sim_whiff_rate | .draw,
        data = posterior_pred %>% filter(.draw <= 25),
        alpha = 0.6, color = ~factor(.draw)) %>%
  gf_labs(title = "Posterior Predictive Distributions of Whiff Rate",
          x = "Simulated Whiff Rate", y = "Density")

```

```{r}
gf_dens(~ whiff_rate, data = fastball_data, color = "blue", size = 1.5) %>%
  gf_dens(~ sim_whiff_rate, group = ~.draw, data = posterior_pred, alpha = 0.2) %>%
  gf_labs(title = "Posterior Predictive vs. Actual Whiff Rate",
          x = "Whiff Rate", y = "Density")

```

First, we gathered 25 random prior pred. samples 
