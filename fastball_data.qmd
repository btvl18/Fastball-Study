...
model {
  ...
  vector[n] beta;
  for ( i in 1:n ) {  
    omega[i] = inv_logit(b0 + b1 * velo[i] + b2 * spin_rate[i]+ b3 * extension[i]+ b4 * (arm_angle*ivb)[i]+ b5 * arm_angle[i]+ b6 * ivb[i]                   );
...
generated quantities {
 // this is not really required - only if you want values of omega, alpha, beta included in your fitted model
  ...
  vector[n] beta;
  for (i in 1:n) {
    omega[i] = inv_logit(b0 + b1 * velo[i] + b2 * spin_rate[i]+ b3 * extension[i]+ b4 * (arm_angle*ivb)[i]+ b5 * arm_angle[i]+ b6 * ivb[i]); // if you want stan to compute modes for each data row       dis line
